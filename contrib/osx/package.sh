...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-CESC.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-cesc-macos
	cd /tmp/electrum-cesc-macos
...
rm -rf /tmp/electrum-cesc-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-cesc-macos/image/
cp -r $1 /tmp/electrum-cesc-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-CESC_uncompressed.dmg \
    /tmp/electrum-cesc-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-CESC_uncompressed.dmg electrum-cesc-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-CESC_uncompressed.dmg

echo "Done."
sha256sum electrum-cesc-$VERSION.dmg
