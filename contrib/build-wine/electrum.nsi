...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-CESC"
  !define PRODUCT_WEB_SITE "https://github.com/ds650/electrum-cesc22"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-cesc\electrum_cesc\gui\icons\electrum.ico"
...
Section
  ...
  File "c:\electrum-ltc\electrum_cesc\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-cesc-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-cesc-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-cesc-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-cesc-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-cesc-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\cryptoescudo" "" "URL:cryptoescudo Protocol"
  WriteRegStr HKCU "Software\Classes\cryptoescudo" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\cryptoescudo" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\cryptoescudo\shell\open\command" "" "$\"$INSTDIR\electrum-cesc-${PRODUCT_VERSION}.exe$\" $\"%1$\""
